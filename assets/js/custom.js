...
$(document).off('click', '.ts-scroll-link').on('click', '.ts-scroll-link', function(e) {
    e.preventDefault();
    const targetId = $(this).data('target');
    const $sidePanel = $('.ts-side-panel');
    const $container = $sidePanel.find('.container-fluid');
    const $target = $container.find('#' + targetId);
    // Open the side panel if not already open
    if (!$('body').hasClass('ts-side-panel-active')) {
        $('.ts-open-side-panel').first().trigger('click');
    }
    // Wait for the panel to be visible, then scroll
    setTimeout(function() {
        if ($target.length) {
            // Calculate offset relative to the scrollable container
            const offset = $target.position().top + $container.scrollTop();
            $container.animate({
                scrollTop: offset
            }, 600);
        }
    ...
    $(document).off('click', '.privacy-policy-trigger').on('click', '.privacy-policy-trigger', function(e) {
        e.preventDefault();
        const myModal = new bootstrap.Modal(document.getElementById('privacyPolicyModal'));
        myModal.show();
    });

    // Terms and Conditions Modal
    $(document).off('click', '.terms-conditions-trigger').on('click', '.terms-conditions-trigger', function(e) {
        e.preventDefault();
        const myModal = new bootstrap.Modal(document.getElementById('termsConditionsModal'));
        myModal.show();
    });